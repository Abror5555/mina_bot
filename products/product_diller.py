...
from database import save_diller_product, get_all_diller_products, delete_diller_product, get_all_categories
...
async def diller_add_product(callback_query: CallbackQuery, state: FSMContext):
    ...
    if user_id == ADMIN:
        ...
        if categories:
            ...
            await state.set_state(product_state.DillerProductState.category)
...
async def select_category_for_diller_product(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Tekshiruv: callback_data dan qaysi qiymat olinmoqda
    print(f"Category split: {category}")
    ...
    await state.set_state(product_state.DillerProductState.name)
    await callback_query.message.delete()
...
async def get_diller_product_name(message: Message, state: FSMContext):
    await state.update_data(name=message.text)
    await message.answer("Mahsulot tavsifini kiriting:")
    await state.set_state(product_state.DillerProductState.description)

# Mahsulot rasmlarini olish
async def get_diller_product_description(message: Message, state: FSMContext):
    await state.update_data(description=message.text)
    await message.answer("Mahsulot uchun rasmlarni yuboring (Jami 4 ta rasm bo'lishi kerak):")
    await message.answer("Maxsulotning brinchi rasmini yuboring")
    await state.set_state(product_state.DillerProductState.image1)

# Birinchi rasmni olish va ikkinchi rasmni so'rash
async def get_diller_product_image1(message: Message, state: FSMContext):
    ...
    await state.set_state(product_state.DillerProductState.image2)

# Ikkinchi rasmni olish va uchinchi rasmni so'rash
async def get_diller_product_image2(message: Message, state: FSMContext):
    ...
    await state.set_state(product_state.DillerProductState.image3)

# Uchinchi rasmni olish va to'rtinchi rasmni so'rash
async def get_diller_product_image3(message: Message, state: FSMContext):
    ...
    await state.set_state(product_state.DillerProductState.image4)

# To'rtinchi rasmni olish va videoni so'rash (majburiy emas)
async def get_diller_product_image4(message: Message, state: FSMContext):
    ...
    await state.set_state(product_state.DillerProductState.video)


# Mahsulot videosini olish
async def get_diller_product_video(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "skip":
        ...
        await message.answer("Maxsulot optim narxini kiriting")
        await state.set_state(product_state.DillerProductState.wholesale_price)
    elif message.video:
...
async def get_diller_price_message_answer(message:Message, state:FSMContext):
        price_text = message.text
        try:
            # Narxni float yoki int ga o'girishga urinib ko'ramiz
            wholesale_price = int(price_text)
            ...
            await message.answer(f"optim narx qabul qilindi: {wholesale_price}")
            await message.answer("Maxsulot complect sonini kiriting")
            
            # Bu yerda holatni yangilab ketishingiz mumkin
            await state.update_data(wholesale_price=wholesale_price)
            await state.set_state(product_state.DillerProductState.stock)
...
async def get_diller_stock_message_answer(message:Message, state:FSMContext):
        ...
        try:
            ...
            if stock <= 0:
                await message.answer("Complect soni 0 dan katta bo'lishi kerak. Iltimos, qaytadan kiriting.")
                return

            # Maxsulot soni to'g'ri formatda bo'lsa, davom etish
            await message.answer(f"Maxsulot complect soni qabul qilindi: {stock}")
            ...
            await state.set_state(product_state.DillerProductState.confirm)
...
async def confirm_diller_product(message: Message, state: FSMContext):
...
async def view_all_diller_products(callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    await callback_query.message.edit_reply_markup(reply_markup=None)
    
    # Faqat admin mahsulotlarni ko'ra olishi uchun shart
    if user_id == ADMIN:
        products = get_all_diller_products()

        if products:
            for product in products:
                # 12 ustunni to'g'ri ajratib olish
                id, name, category_id, description, image1, image2, image3, image4, video, wholesale_price, stock, added_at = product
                ...
                await callback_query.message.answer(
                    f"Kategoriya: {category_id}\nNomi: {name}\nTavsif: {description}\nOptim Narx: {wholesale_price}\n1 Kamplektda: {stock} dona maxsulot bo'ladi\nQo'shilgan sana: {added_at}",
...
async def delete_diller_product_handler(callback_query: CallbackQuery):
    product_id = int(callback_query.data.split("_")[2])  # Callback data format: delete_product_{id}
    callback_query.message.edit_reply_markup(reply_markup=None)

    # Mahsulotni bazadan o'chirish
    user_delete_product = delete_diller_product(product_id)
    print(f"O'chirilgan ustun {user_delete_product}")
    # Javob qaytarish
    await callback_query.answer("Mahsulot muvaffaqiyatli o'chirildi.")
    await callback_query.message.delete()